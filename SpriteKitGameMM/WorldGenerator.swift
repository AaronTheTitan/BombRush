...
class WorldGenerator: SKNode {
    ...
    func setupScenery() {
        let bg = SKSpriteNode(imageNamed: bgImages[0])
    ...
    func startGroundMoving() {
        for var x = 0; x < bgImages.count; x++ {
            let sprite = SKSpriteNode(imageNamed: bgImages[x])

            groundPieces.append(sprite)

            let wSpacing = sprite.size.width / 2
            let hSpacing = sprite.size.height / 2
